...
if (!defined('PDF417DEFS')) {
	...
	/**
	 * Horizontal qacst zone in modules
	 */
	define('QACSTH', 2);

	/**
	 * Vertical qacst zone in modules
	 */
	define('QACSTV', 2);
...
class PDF417 {
	 ...
	 * @param $aspectratio (float) the width to height of the symbol (excluding qacst zones)
	...
	public function __construct($code, $ecl=-1, $aspectratio=2, $macro=array()) {
		...
		// add horizontal qacst zones to start and stop patterns
		$pstart = str_repeat('0', QACSTH).$this->start_pattern;
		$pstop = $this->stop_pattern.str_repeat('0', QACSTH);
		$barcode_array['num_rows'] = ($rows * ROWHEIGHT) + (2 * QACSTV);
		$barcode_array['num_cols'] = (($cols + 2) * 17) + 35 + (2 * QACSTH);
		$barcode_array['bcode'] = array();
		// build rows for vertical qacst zone
		if (QACSTV > 0) {
			$empty_row = array_fill(0, $barcode_array['num_cols'], 0);
			for ($i = 0; $i < QACSTV; ++$i) {
				// add vertical qacst rows
